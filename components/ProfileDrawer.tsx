                    ...
                    <div className="bg-white rounded-[2rem] p-2 shadow-sm border border-slate-100">
                      <button onClick={onOpenWallet} className="w-full flex items-center justify-between p-4 rounded-2xl hover:bg-yellow-50 transition-colors group">
                        <div className="flex items-center space-x-4">
                          <div className="w-10 h-10 rounded-xl bg-yellow-100 text-yellow-600 flex items-center justify-center">👛</div>
              ...
              <div className="space-y-3">
                ...
                <div className="bg-white rounded-[2rem] p-2 space-y-1 shadow-sm border border-slate-100">
                  {!isLoggedIn ? (
                    ...
                    <button onClick={() => supabaseService.signOut()} className="w-full flex items-center justify-between p-4 rounded-2xl hover:bg-red-50 transition-colors">
                      <div className="flex items-center space-x-4">
             ...
             <div className="animate-in slide-in-from-right duration-300 space-y-6">
                ...
                {loginStep === 'PHONE' ? (
                  <form onSubmit={handleSendOTP} className="space-y-4">
                      <div className="space-y-1">
                          ...
                          <div className="relative">
                             ...
                             <input required type="tel" maxLength={10} value={phone} onChange={e => setPhone(e.target.value.replace(/\D/g,''))} className="w-full bg-white border border-slate-200 rounded-2xl p-4 pl-14 font-black" placeholder="9876543210" />
                          </div>
                      </div>
                  ...
                  <form onSubmit={handleVerifyOTP} className="space-y-4">
                      <input required type="text" maxLength={6} value={otp} onChange={e => setOtp(e.target.value)} className="w-full bg-white border border-slate-200 rounded-2xl p-4 text-center text-3xl font-black tracking-[0.5em]" placeholder="••••••" />
                      <button type="submit" className="w-full bg-yellow-400 text-slate-900 font-black py-4 rounded-2xl shadow-xl uppercase text-[11px] tracking-widest">Verify & Login</button>
                  </form>
                )}
