...
import { AgentProfile, WalletTransaction } from '../types';
import { BRAND_NAME, COMMISSION_RATE } from '../constants';
      ...
      <div className="relative w-full max-w-lg bg-white rounded-t-[3rem] h-[90vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom-20 duration-500">
        <div className="p-8 pb-4">
           ...
           <div className="flex justify-between items-start">
              ...
              <button onClick={onClose} className="w-10 h-10 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400">✕</button>
           </div>
        </div>
           ...
           {isWithdrawing ? (
             <form onSubmit={handleWithdrawal} className="space-y-4 animate-in zoom-in-95">
                <div className="space-y-1">
                   <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Amount to Withdraw (Min ₹500)</label>
                   <input required type="number" value={withdrawAmount} onChange={e => setWithdrawAmount(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 text-xl font-black outline-none focus:ring-2 focus:ring-yellow-400" placeholder="₹ 1,000" />
                </div>
                <div className="space-y-1">
                   <label className="text-[10px] font-black uppercase text-slate-400 ml-2">UPI ID (e.g. mobile@upi)</label>
                   <input required type="text" value={upiId} onChange={e => setUpiId(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-4 text-sm font-black outline-none focus:ring-2 focus:ring-yellow-400" placeholder="7200134807@ybl" />
                </div>
                      ...
                      profile.transactions.map((tx) => (
                        <div key={tx.id} className="p-4 bg-slate-50 rounded-3xl flex items-center justify-between group hover:bg-white border border-slate-50 hover:border-slate-100 transition-all">
                           <div className="flex items-center space-x-4">
                              <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm ${tx.type === 'EARNING' ? 'bg-green-100 text-green-600' : tx.type === 'WITHDRAWAL' ? 'bg-blue-100 text-blue-600' : 'bg-red-100 text-red-600'}`}>
